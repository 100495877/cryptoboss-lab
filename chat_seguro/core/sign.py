...
if __name__ == "__main__":
    ...
    print("[TEST] Generando par de claves RSA-3072...")
    ...
    # Mensaje de prueba
    message = b"Hola, este es un mensaje de prueba para firmar"
    
    # Firmar
    print("[TEST] Firmando mensaje...")
    signature = sign_message(private_key, message)
    print(f"  - Firma generada: {format_signature_for_display(signature)}")
    print(f"  - Info: {get_signature_info(signature)}")
    
    # Verificar (correcta)
    print("\n[TEST] Verificando firma correcta...")
    is_valid = verify_signature(public_key, message, signature)
    print(f"  - Resultado: {'✓ VÁLIDA' if is_valid else '✗ INVÁLIDA'}")
    
    # Verificar (mensaje alterado)
    print("\n[TEST] Verificando con mensaje alterado...")
    tampered_message = b"Hola, este es un mensaje ALTERADO"
    is_valid = verify_signature(public_key, tampered_message, signature)
    print(f"  - Resultado: {'✓ VÁLIDA' if is_valid else '✗ INVÁLIDA (esperado)'}")
    
    # Verificar (firma alterada)
    print("\n[TEST] Verificando con firma alterada...")
    tampered_signature = bytearray(signature)
    tampered_signature[0] ^= 0xFF  # Invertir primer byte
    is_valid = verify_signature(public_key, message, bytes(tampered_signature))
    print(f"  - Resultado: {'✓ VÁLIDA' if is_valid else '✗ INVÁLIDA (esperado)'}")
    
    # Test con paquete de mensaje
    print("\n[TEST] Firmando paquete de mensaje completo...")
    nonce = b"123456789012"  # 12 bytes
    ciphertext = b"datos_cifrados_aqui"
    tag = b"0123456789abcdef"  # 16 bytes
    
    package_signature = sign_message_package(private_key, nonce, ciphertext, tag)
    is_valid = verify_message_package(public_key, nonce, ciphertext, tag, package_signature)
    print(f"  - Resultado: {'✓ VÁLIDA' if is_valid else '✗ INVÁLIDA'}")